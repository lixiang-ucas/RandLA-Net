...
class ConfigISPRS:
    ...
    # num_layers = 5  # Number of layers
    num_layers = 4  # Number of layers, default
    
    num_points = 65536//2  # Number of input points, default
    # num_points = 45056
    # num_points = 32768  # Number of input points
    
    num_classes = 9  # Number of valid classes
    # sub_grid_size = 0.2  # preprocess_parameter, default 0.06 
    # sub_grid_size = 0.06 
    sub_grid_size = 0.001 #default
    ...
    learning_rate = 1e-2  # initial learning rate, default
#     learning_rate = 1e-1  # initial learning rate
    # lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate
    lr_decays = {i: 0.9 for i in range(0, 500)}  # decay rate, default

    train_sum_dir = 'train_log/isprs_weakly_v5'
    test_sum_dir = 'test_log/isprs_weakly_v5'
    saving = True
    saving_path = 'results_ISPRS_large_pt_weakly_v5'
    
#     isprs_weakly_baseline: num_points = 65536//2, batch_size=2
#     isprs_weakly_v1: num_points = 65536//2, batch_size=2
#     isprs_weakly_v2: num_points = 65536//2, batch_size=2, no smoothness loss

#     isprs_weakly_v0: num_points = 65536//2, batch_size=2, no weakly loss
#     isprs_weakly_v3: num_points = 65536//2, batch_size=2, ramup=20
#     isprs_weakly_v4: num_points = 65536//2, batch_size=2, ramup=20, mask=0.01

#     isprs_weakly_v5: num_points = 65536//2, batch_size=2, ramup=20, new class balance
...
class ConfigDFC:
    ...
    k_n = 16  # KNN
    num_layers = 5  # Number of layers
    num_points = 65536//2  # Number of input points, default=20480
    ...
    train_steps = 500  # Number of steps per epochs
    val_steps = 800  # Number of validation steps per epoch
    ...
    train_sum_dir = 'train_log/dfc_weakly_v5' #0.006
    test_sum_dir = 'test_log/dfc_weakly_v5'
    saving = True
    saving_path = 'results_DFC_weakly_v5'
...
class DataProcessing:
    ...
    @staticmethod
    def data_aug(xyz, color, labels, idx, num_out, masks=None):
        num_in = len(xyz)
        ...
        masks_aug = masks[idx_dup]
        if masks is not None:
            return xyz_aug, color_aug, idx_aug, label_aug, masks_aug
        else:
    ...
    @staticmethod
    def grid_sub_sampling(points, features=None, labels=None, grid_size=0.1, verbose=0, masks=None):
        """
    ...
    def get_class_weights(dataset_name):
        ...
        elif dataset_name is 'ISPRS':
            num_per_class = np.array([1146, 279669, 295843, 8322,  19499, 261213,  38492,  72462, 189482])
        elif dataset_name is 'DFC':
            num_per_class = np.array([3582055, 48067986, 10774877, 10269443, 1356857, 934532])
        
#         if  dataset_name is 'ISPRS':
#             weight = num_per_class / float(sum(num_per_class))
#             ce_label_weight = 1/np.log(1.2+weight)
        if dataset_name is 'DFC':
            weight = num_per_class[1:] / float(sum(num_per_class[1:]))
            ce_label_weight = 1/np.log(1.2+weight)
            ce_label_weight = np.insert(ce_label_weight, 0, 1)
        else:
            weight = num_per_class / float(sum(num_per_class))
