...
sub_grid_size = 0.001
original_pc_folder = join(dirname(dataset_path), 'original_ply_wh_mask0.01')
sub_pc_folder = join(dirname(dataset_path), 'input_{:.3f}_wh_mask0.01'.format(sub_grid_size))
...
trainFeats = trainSet[:,[3,5,6]] #use reflectance, height above ground, return_number
...
####create mask index]
masks = np.zeros((len(xyz),1), dtype=bool)
mask_idx = np.random.choice(len(xyz), int(0.01*len(xyz)), replace=False)
masks[mask_idx] = True
...
trainFeats = trainSet[:,[6,3,4]] #height, reflectance, retuan number
colors = trainFeats.astype(np.float32)
