...
#                 self.saving_path = self.config.saving_path
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            # f_encoder_i = self.transformer_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],
            #                                      'Encoder_layer_' + str(i), is_training)
            f_encoder_i = self.multihead_transformer_block(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],
                                     'Encoder_layer_' + str(i), 3, is_training)
            f_sampled_i = self.random_sample(f_encoder_i, inputs['sub_idx'][i]) #(B,N,d), (B,N',k)
            feature = f_sampled_i
            if i == 0:
        ...
        for j in range(self.config.num_layers):
            ...
            f_decoder_i = helper_tf_util.conv2d_transpose(tf.concat([f_encoder_list[-j - 2], f_interp_i], axis=3),
                                                          ...
                                                          is_training=is_training) #already use skip-connections
            feature = f_decoder_i
    ...
    def transformer_block(self, feature, xyz, neigh_idx, d_out, name, is_training):
        print('feature, xyz, neigh_idx, d_out', feature.shape, xyz.shape, neigh_idx.shape, d_out)
        # print('feature, xyz, neigh_idx, d_out', tf.shape(feature), tf.shape(xyz), tf.shape(neigh_idx), tf.shape(d_out))
        f_pc = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training) # (B,N,1,16)
        # print("f_pc:", f_pc.shape, tf.shape(f_pc), tf.shape(f_pc[0]))
        f_phi = helper_tf_util.conv2d(f_pc, d_out, [1, 1], name + 'phi', [1, 1], 'VALID', True, is_training)
        f_psi = helper_tf_util.conv2d(f_pc, d_out, [1, 1], name + 'psi', [1, 1], 'VALID', True, is_training)
        f_alpha = helper_tf_util.conv2d(f_pc, d_out, [1, 1], name + 'alpha', [1, 1], 'VALID', True, is_training)
        f_phi_expanded = tf.tile(f_phi, [1, 1, tf.shape(neigh_idx)[-1], 1])
        f_psi_expanded = self.gather_neighbour(tf.squeeze(f_psi, axis=2), neigh_idx)
        f_alpha_expanded = tf.tile(f_alpha, [1, 1, tf.shape(neigh_idx)[-1], 1])
        # print("tf.shape(neigh_idx)[-1]", tf.shape(neigh_idx)[-1])
        # print("f_phi_expanded:", f_phi_expanded.shape, tf.shape(f_phi_expanded), tf.shape(f_phi_expanded[0]), tf.shape(f_phi_expanded[0][0]))
        # print("f_psi_expanded:", f_psi_expanded.shape, tf.shape(f_psi_expanded), tf.shape(f_phi_expanded[0]), tf.shape(f_phi_expanded[0][0]))
        # print("f_alpha_expanded:", f_alpha_expanded.shape, tf.shape(f_alpha_expanded), tf.shape(f_phi_expanded[0]), tf.shape(f_phi_expanded[0][0]))

        neighbor_xyz = self.gather_neighbour(xyz, neigh_idx)
        # print("neighbor_xyz:", neighbor_xyz.shape)
        xyz_tile = tf.tile(tf.expand_dims(xyz, axis=2), [1, 1, tf.shape(neigh_idx)[-1], 1])
        relative_xyz = xyz_tile - neighbor_xyz
        f_delta = helper_tf_util.conv2d(relative_xyz, d_out // 2, [1, 1], name + 'delta1', [1, 1], 'VALID', True, is_training)
        # print(f_delta.shape)
        f_delta = helper_tf_util.conv2d(f_delta, d_out, [1, 1], name + 'delta2', [1, 1], 'VALID', True, is_training)
        # print(f_delta.shape)
        f_delta = tf.nn.leaky_relu(f_delta)
        
        gamma_in = f_phi_expanded - f_psi_expanded + f_delta
        # import pdb;pdb.set_trace()
        # print(gamma_in.shape)
        f_gamma = helper_tf_util.conv2d(gamma_in, d_out // 2, [1, 1], name + 'gamma1', [1, 1], 'VALID', True, is_training)
        f_gamma = helper_tf_util.conv2d(f_gamma, d_out, [1, 1], name + 'gamma2', [1, 1], 'VALID', True, is_training)
        f_gamma = tf.nn.leaky_relu(f_gamma)
        att_map = tf.nn.softmax(f_gamma, axis=2) #(B,N,K,D)->(B,N,K,D)
        
        f_out = att_map * (f_alpha_expanded + f_delta) #(B,N,K,D)*(B,N,K,D)
        f_out = tf.reduce_sum(f_out, 2)
                           
        # f_out = helper_tf_util.conv2d(tf.expand_dims(f_out, axis=2), d_out * 2, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training, activation_fn=None)
        # shortcut = helper_tf_util.conv2d(feature, d_out * 2, [1, 1], name + 'shortcut', [1, 1], 'VALID',
        #                                  activation_fn=None, bn=True, is_training=is_training)
        # return tf.nn.leaky_relu(f_out + shortcut)

        f_out = helper_tf_util.conv2d(tf.expand_dims(f_out, axis=2), d_out, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training, activation_fn=None)
        # shortcut = helper_tf_util.conv2d(feature, d_out * 2, [1, 1], name + 'shortcut', [1, 1], 'VALID',
                                         # activation_fn=None, bn=True, is_training=is_training)
        return tf.nn.leaky_relu(f_out + f_pc)
    
    def multihead_transformer_block(self, feature, xyz, neigh_idx, d_out, name_0, num_heads, is_training):
        out_multis = []
        for k in range(num_heads):
            name = name_0 + '_'+str(k)+'_'
            print('feature, xyz, neigh_idx, d_out', feature.shape, xyz.shape, neigh_idx.shape, d_out)
            f_pc = helper_tf_util.conv2d(feature, d_out, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
            ...
            f_out = helper_tf_util.conv2d(tf.expand_dims(f_out * 2, axis=2), d_out * 2, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training, activation_fn=None)
            shortcut = helper_tf_util.conv2d(feature, d_out * 2, [1, 1], name + 'shortcut', [1, 1], 'VALID',
                                             activation_fn=None, bn=True, is_training=is_training)
            out = tf.nn.leaky_relu(f_out + shortcut)
            out_multis.append(out)
        out_multis = tf.concat(out_multis, -1) #(B,N,num_heads*2d_out)
        return out_multis
    ...
    def random_sample(feature, pool_idx):
        ...
        pool_features = tf.batch_gather(feature, pool_idx) #(B,N,d), (B, n'*k)->(B,n'*k,d)
        pool_features = tf.reshape(pool_features, [batch_size, -1, num_neigh, d])
        pool_features = tf.reduce_max(pool_features, axis=2, keepdims=True) #(B,n'*k,d)->(B,n',d)
