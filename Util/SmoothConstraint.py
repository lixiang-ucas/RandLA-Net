...
def Loss_SpatialColorSmooth_add_SelfContain_mustLink(Z,X,M,Y,gamma=1e-1,knn=10):
    '''
    function to return spatial and color smoothness constraint loss
    :param Z: Input point cloud feature embedding float B*N*D
    :param X: Input point cloud  float B*N*6    XYZRGB
...
#     W_knn = tf.stack(W_knn)     # B*N*knn
    ...
    ## Compute Knn Graph version 2
    Ind = tf.py_func(DP.knn_search, [X, X, knn], tf.int32) # B*N*knn
    Ind = tf.reshape(Ind, [B,N,knn])
    X_tilde = Tool.batch_gather_v1(X,Ind)   # indexed X B*N*Knn*D
    X_expand = tf.tile(X[:,:,tf.newaxis,:],[1,1,knn,1])    # expanded X B*N*Knn*D
    Dis = tf.reduce_mean((X_expand - X_tilde)**2,axis=-1) #B*N*Knn
    W_knn = tf.exp(-Dis/gamma) #B*N*Knn
    
    #############end
    
    y_i = tf.tile(tf.expand_dims(tf.cast(Y, tf.float32), -1), [1,1,knn])
    y_j = tf.reshape(tf.batch_gather(tf.cast(Y, tf.float32), tf.reshape(Ind, (B, N*knn))), (B,N,knn))
    eq_y_idx = tf.cast(tf.equal(y_i, y_j), tf.float32)
    neq_y_idx = tf.cast(tf.not_equal(y_i, y_j), tf.float32)

    mi_idx =tf.tile(tf.expand_dims(tf.cast(M, tf.float32), -1), [1,1,knn])
    mj_idx = tf.reshape(tf.batch_gather(tf.cast(M, tf.float32), tf.reshape(Ind, (B, N*knn))), (B,N,knn))
    must_link_idx = mi_idx * mj_idx * eq_y_idx
    must_not_link_idx = mi_idx * mj_idx * neq_y_idx
    
    W_knn = tf.where(tf.cast(must_link_idx, tf.bool), tf.ones_like(W_knn), W_knn)
    W_knn = tf.where(tf.cast(must_not_link_idx, tf.bool), -1*tf.ones_like(W_knn), W_knn)
