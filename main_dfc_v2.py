...
from RandLANet_weakly_v2 import Network
from tester_ISPRS import ModelTester
from helper_ply import read_ply
from helper_tool import ConfigDFC as cfg
...
#         stacked_features = tf.concat([transformed_xyz, rgb], axis=-1)
    ...
    def get_tf_mapping2(self):
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_masks, batch_pc_idx, batch_cloud_idx):
            ...
            transformed_xyz = tf.map_fn(self.tf_augment_input, [batch_xyz, batch_features], dtype=tf.float32)
            
            batch_features = tf.concat([batch_xyz, batch_features], axis=-1)
            batch_features_trans = tf.concat([transformed_xyz, batch_features], axis=-1)
            
            batch_xyz = tf.stack([batch_xyz, transformed_xyz], axis=0)
            batch_features = tf.stack([batch_features, batch_features_trans], axis=0)
            
            ## Prepare Labels
            batch_labels = tf.stack([batch_labels, batch_labels], axis=0)
            batch_masks = tf.stack([batch_masks, batch_masks], axis=0)
            batch_pc_idx = tf.stack([batch_pc_idx, batch_pc_idx], axis=0)
            batch_cloud_idx = tf.stack([batch_cloud_idx, batch_cloud_idx], axis=0)
...
# $(which python) main_dfc_v2.py --gpu 3 --mode train

# $(which python) main_dfc.py --gpu 1 --mode test
...
if __name__ == '__main__':
    ...
    os.system('cp main_dfc.py {}'.format(saving_path))
