...
#         pdb.set_trace()
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ops = [self.train_op,
                       ...
                       self.mask_boolen,
                       self.accuracy]
                _, _, summary, l_out, probs, labels, _, acc = self.sess.run(ops, {self.is_training: True})
    ...
    def evaluate(self, dataset):
        ...
        for step_id in range(self.config.val_steps):
            ...
            try:
                ...
                summary, stacked_prob, labels, acc = self.sess.run(ops, {self.is_training: False})
    ...
    def WeakSupLoss(self):
        ...
        batch_size = int(tf.shape(self.batch_labels)[0]//2)
#         one_hot_labels = tf.reshape(one_hot_labels, [self.inputs.shape[0],self.inputs.shape[1],self.config.num_classes])
        L_gt = tf.cast(tf.reduce_max(one_hot_labels[:batch_size], axis=1), tf.float32)  # inexact labels B*13
        L_pred = tf.reduce_max(self.batch_logits[:batch_size], axis=1)
        ...
        self.loss_siamese = 1* tf.reduce_mean(tf.reduce_sum((self.prob_logits[:batch_size] - self.prob_logits[batch_size:]) ** 2, axis=-1))
...
#         self.loss_smooth = 10* SmoothConstraint.Loss_SpatialColorSmooth_add_SelfContain(self.prob_logits[0::2], self.inputs['features'][0::2, :, 0:3], M=self.batch_masks[0::2], Y=self.batch_labels[0::2], gamma=1e-1)
        self.loss_smooth = 10 * SmoothConstraint.Loss_SpatialColorSmooth_add_SelfContain_mustLink(self.prob_logits[:batch_size], self.inputs['features'][:batch_size, :, 0:6], M=self.batch_masks[:batch_size], Y=self.batch_labels[:batch_size], gamma=1e-1)#0:5
